...
class Game {
    ...
    requestPresidentChooseChancellor() {
        let cloneArr = this.connectedPlayers;
    ...
    requestVoteOnPair(data) {
        ...
        callbacks[callbackID] = (data) => {
            ...
            console.log(data.id)
            let player = this.connectedPlayers.map(_player => {
            ...
            if (data.value == 0) {
                console.log(player.name + ' voted ja')
                this.addYay(player);
            } else {
                console.log(player.name + ' voted nein')
                this.addNay(player);
            }
        }
    ...
    addNay(player) {
        ...
        if (this.nay + this.yay >= this.connectedPlayers.length) {
            ...
            io.to(player.id).emit('waiting event', { title: 'Waiting on other players to vote', excluded: null })
        }
    ...
    endVote() {
        ...
        if (this.yay > this.nay) {
            ...
            if (this.currentChancellor.id == this.hitler.id && this.fascistPolicies >= 3) {
                ...
                console.log("end of game not triggered: Chancellor id = " + this.currentChancellor.id + ", hitler.id = " + hitler.id);
            }
            io.to(this.roomname).emit('waiting event', { title: 'President ' + this.currentPresident.name + ' and Chancellor ' + this.currentChancellor.name + ' were elected' });
            // setTimeout(this.runPolicyTurn, 5000);

        }
        else if (this.nay >= this.yay) {
            console.log("skip");
            io.to(this.roomname).emit('waiting event', { title: 'President ' + this.currentPresident.name + ' and Chancellor ' + this.currentChancellor.name + ' were not elected' });
