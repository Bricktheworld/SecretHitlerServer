...
class Game {
    ...
    requestNominatedPresidentChooseChancellor(player) {
        let possibleChancellors = this.connectedPlayers.slice(0);
        let nominatedPresident;

        for (i = 0; i < possibleChancellors.length; i++) {
            if (player.id == possibleChancellors[i].id) {
                possibleChancellors.splice(i, 1)
                nominatedPresident = possibleChancellors[i]
            }
        }
        let callbackID = shortid.generater();
        callbacks[callbackID] = (data) => {
            io.to(nominatedPresident.id).emit('action task completed')
            this.requestVoteOnPair(data)
        }
        io.to(nominatedPresident.id).emit('pick chancellor', { data: possibleChancellors, callbackb: callbackID, title: 'You have been chosen as president for a special election, choose a chancellor' });
        io.to(this.roomname).emit('waiting event', { title: 'President chose ' + nominatedPresident.name + ' to be president', excluded: nominatedPresident.id })
    }
    ...
    requestInvestigation() {
        ...
        if (this.excludedInvestigatedPlayer != null) {
            for (let i = 0; i < investigatablePlayers.length; i++) {
                if (investigatablePlayers[i].id == this.excludedInvestigatedPlayer || investigatablePlayers[i].id == this.currentPresident.id)
    ...
    investigatePlayer(data) {
        ...
        for (let i = 0; i < this.connectedPlayers.length; i++) {
            if (data.id == this.connectedPlayers[i].id) {
                this.excludedInvestigatedPlayer = this.connectedPlayers[i];
                player = this.connectedPlayers[i]
            }
        }
    ...
    beginSpecialElection = () => {
        let possiblePresidents = this.connectedPlayers.slice(0)
        ...
        callbacks[callbackID] = () => {
            this.requestNominatedPresidentChooseChancellor();
            io.to(this.currentPresident.id).emit('action task completed')
        }
        io.to(this.currentPresident.id).emit('Choose player for special election', { title: 'Choose a president for a special election', data: possiblePresidents, callback: callbackID });
