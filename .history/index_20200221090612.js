    ...
    requestNominatedPresidentChooseChancellor(player) {
        ...
        for (i = 0; i < possibleChancellors.length; i++) {
            if (player.id == possibleChancellors[i].id) {
                ...
                nominatedPresidentte = possibleChancellors[i]
            }
        }
        ...
        io.to(nominatedPresidente.id).emit('pick chancellor', { data: possibleChancellors, callbackb: callbackID, title: 'You have been chosen as president for a special election, choose a chancellor' });
        io.to(this.roomname).emit('waiting event', {})
    }
