...
class Game {
    ...
    requestPresidentChooseChancellor() {
        ...
        if (this.currentChancellor == null && this.currentPresident != null) {
            cloneArr.splice(this.connectedPlayers.indexof(this.currentPresident))
        } else {
            cloneArr.splice(this.connectedPlayers.indexOf(this.currentPresident))
            cloneArr.splice(this.connectedPlayers.indexOf(this.currentChancellor))
            console.log("excluded chancellor: " + this.currentChancellor.id);
            if (this.connectedPlayers.length > 5 && this.excludedPresident != null) {
                cloneArr.splice(this.connectedPlayers.indexOf(this.excludedPresident))
                console.log("excluded president: " + this.excludedPresident.id);
            }
        }
        io.to(this.currentPresident.id).emit('pick chancellor', cloneArr);
    }
