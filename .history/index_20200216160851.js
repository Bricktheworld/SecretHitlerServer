...
class Game {
    ...
    initDeck() {
        ...
        for (let i = 0; i < 6; i++) {
            policies.push({ value: 0, id: shortid.generate() });
        }
        policies = shuffleArray(policies);
        console.log(policies);
        for (let i = 0; i < policies.length; i++) {
            console.log(policies[i]);
        }

        drawPolicies = policies;
    }
    ...
    endVote() {
        ...
        if (this.yay > this.nay) {
            ...
            setTimeout(this.beginNewTurn, 5000);
    ...
    beginNewTurn() {
        ...
        io.to(this.currentPresident.id).emit('president discard policy', this.hand);
    }
}
