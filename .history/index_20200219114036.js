...
class Game {
    constructor(gamePin, owner, ownerID, name, numberOfPlayers, dataObject) {
        ...
        this.livingPlayers = [{ name: owner, id: ownerID, role: -1, spectator: false, dead: false, }];
        this.deadPlayers = [];
        this.fascists = [];
        this.liberals = [];
        this.hitler;
    ...
    addUser(data) {
        ...
        this.livingPlayers.push(data)
        io.to(this.roomname).emit('update players in lobby', this.livingPlayers)
    }

    disconnectUser(data) {
        for (let i = 0; i < this.livingPlayers.length; i++) {
            if (this.livingPlayers[i].id == data) {
                this.livingPlayers.splice(i, 1);
                io.to(this.roomname).emit('update players in lobby', this.livingPlayers)
                if (this.ownerID == data && this.livingPlayers.length > 0) {
                    this.reassignOwner()
    ...
    reassignOwner() {
        let indexOfNewOwner = Math.floor(Math.random() * this.livingPlayers.length)
        this.owner = this.livingPlayers[indexOfNewOwner].name;
        this.ownerID = this.livingPlayers[indexOfNewOwner].id;
    ...
    startGame() {
        ...
        for (let i = 0; i < this.livingPlayers.length; i++) {
            ...
            io.to(this.livingPlayers[i].id).emit('assign role', (this.connectedPlayers[i].role))
        ...
        this.livingPlayers = shuffleArray(this.connectedPlayers)

        this.initDeck();
        io.to(this.roomname).emit('start game', this.livingPlayers);
        setTimeout(() => {
            for (let i = 0; i < this.livingPlayers.length; i++) {
                io.to(this.livingPlayers[i].id).emit('role sync', this.connectedPlayers[i].role)
