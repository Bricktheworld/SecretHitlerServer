...
class Game {
    ...
    pickPresident() {
        ...
        console.log("current president: " + this.currentPresident.username)
        io.to(this.roomname).emit('president nominating chancellor', this.currentPresident);
    ...
    requestPresidentChooseChancellor() {
        ...
        if (this.currentChancellor == null && this.currentPresident != null) {
            cloneArr.splice(indexof(this.currentPresident))
        } else {
            let data = [this.currentChancellor.id];
            cloneArr.splice(indexOf(this.currentPresident))
            cloneArr.splice(indexOf(this.currentChancellor))
            console.log("excluded: " + this.currentChancellor.id);
            if (this.connectedPlayers.length > 5 && this.excludedPresident != null) {
                cloneArr.splice(indexOf(this.excludedPresident))
                console.log("excluded president: " + this.excludedPresident.id);

            }
        }
        io.to(this.currentPresident.id).emit('pick chancellor');
    }
