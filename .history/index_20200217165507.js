...
class Game {
    ...
    requestPresidentChooseChancellor() {
        ...
        if (this.currentChancellor == null && this.currentPresident != null) {
            ...
            cloneArr.splice(cloneArr.indexOf(this.currentPresident), 1)
            if (cloneArr.indexOf(this.currentChancellor) != -1) {
                cloneArr.splice(cloneArr.indexOf(this.currentChancellor), 1)
                console.log("excluded chancellor: " + this.currentChancellor.id);
            }
            if (this.connectedPlayers.length > 5 && this.excludedPresident != null) {
                cloneArr.splice(cloneArr.indexOf(this.excludedPresident), 1)
    ...
    requestVoteOnPair(data) {
        ...
        this.yayers = [];
        this.nayers = [];
        this.nominatedChancellor = this.connectedPlayers.map((player) => { if (player.id == data.id) return player });
        let candidates = [];
