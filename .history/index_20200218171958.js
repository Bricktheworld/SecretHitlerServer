...
class Game {
    ...
    requestInvestigation() {
        if (this.excludedInvestigatedPlayer != null) {
            for (i = 0; i < this.connectedPlayers.length; i++) {
                if (this.connectedPlayers[i].id == this.excludedInvestigatedPlayer)
        }
        }


        if (this.excludedInvestigatedPlayer == null) {
            io.to(this.currentPresident.id).emit('investigate player', { title: 'examine player' });
        } else {
