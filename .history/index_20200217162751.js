...
class Game {
    ...
    requestPresidentChooseChancellor() {
        ...
        if (this.currentChancellor == null && this.currentPresident != null) {
            cloneArr.splice(this.connectedPlayers.foreach((player, index) => { if (player.id == this.currentPresident.id) return index }), 1)
        } else {
            cloneArr.splice(this.connectedPlayers.foreach((player, index) => { if (player.id == this.currentPresident.id) return index }), 1)
            cloneArr.splice(this.connectedPlayers.foreach((player, index) => { if (player.id == this.currentChancellor.id) return index }), 1)
            console.log("excluded chancellor: " + this.currentChancellor.id);
            if (this.connectedPlayers.length > 5 && this.excludedPresident != null) {
                cloneArr.splice(this.connectedPlayers.foreach((player, index) => { if (player.id == this.excludedPresident.id) return index }), 1)
                console.log("excluded president: " + this.excludedPresident.id + ', player length: ' + this.connectedPlayers.length);
            }
        }
