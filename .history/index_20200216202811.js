...
class Game {
    ...
    discardPolicy(data) {
        ...
        io.to(this.currentPresident.id).emit('waiting event', { title: 'Waiting for chancellor to place policy', excluded: 'none' })
    ...
    requestKillAPlayer() {
        ...
        io.to(this.currentPresident.id).emit('killPlayer');
    ...
    runSpecialAbilityCheck() {
        ...
        } else if (expectedLength == 7 || expectedLength == 8) {
    ...
    examineTopThreeCards() {
        ...
        io.to(this.currentPresident.id).emit('examine top three cards', { data: this.cards, title: 'These are the next 3 cards in the deck' });
